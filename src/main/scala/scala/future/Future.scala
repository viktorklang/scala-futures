...
trait Future[+T] extends Awaitable[T] {
  ...
  def onSuccess[U](pf: PartialFunction[T, U])(implicit executor: ExecutionContext): Unit = onComplete {
    t => if (t.isInstanceOf[Success[T]]) pf.applyOrElse[T, Any](t.asInstanceOf[Success[T]].value, impl.Future.id[T])
  }
  ...
  def onFailure[U](@deprecatedName('callback) pf: PartialFunction[Throwable, U])(implicit executor: ExecutionContext): Unit = onComplete {
      t => if(t.isInstanceOf[Failure[T]]) pf.applyOrElse[Throwable, Any](t.asInstanceOf[Failure[T]].exception, impl.Future.id[Throwable])
  }
  ...
  def failed: Future[Throwable] =
    transform({
      t =>
        ...
        else impl.Future.failedFailure
    })(internalExecutor)
    ...
    transform {
      t =>
        if (t.isInstanceOf[Success[T]]) {
          ...
          else impl.Future.filterFailure
        } else t
    }
  ...
  def collect[S](pf: PartialFunction[T, S])(implicit executor: ExecutionContext): Future[S] =
    transform {
      t =>
        if (t.isInstanceOf[Success[T]])
          Success(pf.applyOrElse(t.asInstanceOf[Success[T]].value, impl.Future.collectFailed))
    ...
    transformWith {
      t =>
        if (t.isInstanceOf[Failure[T]]) {
          val result = pf.applyOrElse(t.asInstanceOf[Failure[T]].exception, impl.Future.recoverWithFailed)
          if (result ne impl.Future.recoverWithFailedMarker) result
          else this
        } else this
    }
  ...
  def zip[U](that: Future[U]): Future[(T, U)] =
    zipWith(that)(impl.Future.zipWithTuple2)(internalExecutor)
  ...
  def mapTo[S](implicit tag: ClassTag[S]): Future[S] = {
    ...
    val boxedClass = {
      ...
      if (c.isPrimitive) impl.Future.toBoxed(c) else c
    }
  ...
  def andThen[U](pf: PartialFunction[Try[T], U])(implicit executor: ExecutionContext): Future[T] =
    transform {
      result =>
        try pf.applyOrElse[Try[T], Any](result, impl.Future.id[Try[T]])
...
object Future {
   ...
   */
  final val unit: Future[Unit] = Promise.fromTry(Success(())).future
  ...
  */
  ...
  def sequence[A, M[X] <: TraversableOnce[X]](in: M[Future[A]])(implicit cbf: CanBuildFrom[M[Future[A]], A, M[A]], executor: ExecutionContext): Future[M[A]] = {
    in.foldLeft(successful(cbf(in))) {
      (fr, fa) => fr.zipWith(fa)(impl.Future.addToBuilderFun)
  ...
  def firstCompletedOf[T](futures: TraversableOnce[Future[T]])(implicit executor: ExecutionContext): Future[T] =
    if (futures.isEmpty) Future.failed[T](impl.Future.firstCompletedOfException)
    else {
    val p = Promise[T]()
  ...
  def traverse[A, B, M[X] <: TraversableOnce[X]](in: M[A])(fn: A => Future[B])(implicit cbf: CanBuildFrom[M[A], B, M[B]], executor: ExecutionContext): Future[M[B]] =
    in.foldLeft(successful(cbf(in))) {
      (fr, a) => fr.zipWith(fn(a))(impl.Future.addToBuilderFun)
    }.map(_.result())(InternalCallbackExecutor)
